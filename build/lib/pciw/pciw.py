...
class __data__:
    nt_memory_type = [None, "Other", "DRAM", "Synchronous DRAM", "Cache DRAM", "EDO", "EDRAM", "VRAM", "SRAM", "RAM", "ROM", "Flash",
                      "EEPROM", "FEPROM", "EPROM", "CDRAM", "3DRAM", "SDRAM", "SGRAM", "RDRAM", "DDR", "DDR2", "DDR2 FB-DIMM", None, "DDR3", "FBD2", "DDR4"]
    nt_memory_form_factor = [None, "Other", "SIP", "DIP", "ZIP", "SOJ", "Proprietary", "SIMM", "DIMM", "TSOP", "PGA",
                             "RIMM", "SODIMM", "SRIMM", "SMD", "SSMP", "QFP", "TQFP", "SOIC", "LCC", "PLCC", "BGA", "FPBGA", "LGA", "FB-DIMM"]
    nt_video_memory_type = {1: "Other", 2: None, 3: "VRAM", 4: "DRAM", 5: "SRAM", 6: "WRAM", 7: "EDO RAM",
                            8: "Burst Synchronous DRAM", 9: "Pipelined Burst SRAM", 10: "CDRAM", 11: "3DRAM", 12: "SDRAM", 13: "SGRAM"}
    nt_video_architecture = {1: "Other", 2: None, 3: "CGA", 4: "EGA", 5: "VGA", 6: "SVGA", 7: "MDA",
                             8: "HGC", 9: "MCGA", 10: "8514A", 11: "XGA", 12: "Linear Frame Buffer", 160: "PC-98"}
    nt_video_availability = {1: "other", 2: None, 3: "works", 4: "warning", 5: "tested", 6: "not_applicable", 7: "power_off", 8: "stand-alone_operation", 9: "not_serve", 10: "degradation",
                             11: "not_established", 12: "installation_error", 13: "energy_saving_not_data", 14: "energy_saving_low_consumption_mode", 15: "energy_saving_standby_mode", 16: "power_cycle", 17: "energy_saving_warning"}
...
class __types_nt__:
    ...
    memory_card = namedtuple("memory_card", [
                             "model", "type", "form_factor", "capacity", "frequency", "serial_number", "data_width", "location"])
    monitor = namedtuple(
        "monitor", ["name", "primary", "size", "size_mm", "position"])
    videocard = namedtuple("videocard", [
                           "name", "processor", "memory", "company", "availability", "driver_date", "driver_version"])
    videocard_memory = namedtuple("videocard_memory", ["type", "size"])
    videocard_processor = namedtuple(
        "videocard_processor", ["name", "architecture"])
    motherboard = namedtuple("motherboard", [
                             "name", "product", "manufacturer", "serial_number", "version", "tag", "status", "features"])
    motherboard_status = namedtuple(
        "motherboard_status", ["powered_on", "hosting_board"])
    motherboard_features = namedtuple("motherboard_features", [
                                      "removable", "replaceable", "requires_daughter_board", "hot_swappable"])
    bios = namedtuple("bios", ["name", "manufacturer", "primary", "version", "release_date",
                      "serial_number", "language_current", "language_supported", "characteristics", "smbios"])
    smbios = namedtuple(
        "smbios", ["version", "major_version", "minor_version", "present"])
...
class __func__:
    ...
    def values_str_to_dict(string: str) -> list[dict[str, Union[str, None]]]:
        data: list[list[str]] = [i.split("\r\r\n") for i in __func__.delect_values_list(
            string.replace("\r\r\n\r\r\n\r\r\n\r\r\n", "").split("\r\r\n\r\r\n"), [""])]
        do = []
        for table in data:
            d = {}
    ...
    def get_memory_info_nt() -> list[__types_nt__.memory_card]:
        data = __func__.values_str_to_dict(subprocess.check_output(
            "wmic memorychip get devicelocator,partnumber,serialnumber,capacity,speed,datawidth,formfactor,memorytype /FORMAT:value").decode(errors="ignore"))
        do = []
        for i in data:
            do.append(
    ...
    def get_monitors_info_nt() -> list[__types_nt__.monitor]:
        ...
        for monotor_data in screeninfo.get_monitors():
            displays.append(__types_nt__.monitor(monotor_data.name.replace("\\", "").replace(".", ""), monotor_data.is_primary, [
                            monotor_data.width, monotor_data.height], [monotor_data.width_mm, monotor_data.height_mm], [monotor_data.x, monotor_data.y]))
        return displays

    def get_videocards_info_nt() -> list[__types_nt__.videocard]:
        data = __func__.values_str_to_dict(subprocess.check_output(
            "wmic path win32_VideoController get Name,VideoProcessor,AdapterRAM,Availability,AdapterCompatibility,VideoArchitecture,VideoMemoryType,DriverDate,DriverVersion /FORMAT:value").decode(errors="ignore"))
        do = []
        for i in data:
            do.append(
                __types_nt__.videocard(
                    ...
                    __types_nt__.videocard_processor(
                        str(i["Name"]),
                        __data__.nt_video_architecture[int(
                            i["VideoArchitecture"])]
                    ),
                    __types_nt__.videocard_memory(
                        __data__.nt_video_memory_type[int(
                            i["VideoMemoryType"])],
                        int(i["AdapterRAM"])
                    ),
    ...
    def get_motherboard_info_nt() -> __types_nt__.motherboard:
        i = __func__.values_str_to_dict(subprocess.check_output(
            "wmic BASEBOARD get Name,PoweredOn,Product,Removable,Replaceable,RequiresDaughterBoard,SerialNumber,Tag,Version,HostingBoard,HotSwappable,Manufacturer /FORMAT:VALUE").decode(errors="ignore"))[0]
    ...
    def get_bios_info_nt() -> __types_nt__.bios:
        i = __func__.values_str_to_dict(subprocess.check_output(
            "wmic bios list full").decode(errors="ignore"))[0]
        characteristics_ids, characteristics = list(
            eval(str(i["BiosCharacteristics"]))), []
...
class CPU:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "cache": {
                "size": self.cache.size._asdict(),
                "line_size": self.cache.line_size._asdict(),
                "associativity": self.cache.associativity._asdict()
            },
            "flags": self.flags
        }
...
class RAM:
    ...
    def to_dict(self) -> list[dict]:
        return [item._asdict() for item in self.memory_cards]
...
class GPU:
    ...
    def to_dict(self) -> list[dict]:
        return [
            {
                "name": i.name,
                "processor": i.processor._asdict(),
                "memory": i.memory._asdict(),
                "company": i.company,
                "availability": i.availability,
                "driver_date": i.driver_date.timestamp(),
...
class Motherboard:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "tag": self.info.tag,
            "status": self.info.status._asdict(),
            "features": self.info.features._asdict()
        }
...
class Monitors:
    ...
    def to_dict(self) -> list[dict]:
        return [i._asdict() for i in self.displays]
...
class BIOS:
    ...
    def to_dict(self) -> dict:
        return {
            "name": self.info.name,
            "manufacturer": self.info.manufacturer,
            "primary": self.info.primary,
            "version": self.info.version,
            "release_date": self.info.release_date.timestamp(),
            "serial_number": self.info.serial_number,
            "language_current": self.info.language_current,
            "language_supported": self.info.language_supported,
            "characteristics": self.info.characteristics,
            "smbios": self.info.smbios._asdict(),
        }
